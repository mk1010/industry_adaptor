...
export DOCKER_HOST_IP = $(shell ifconfig eth0 | grep inet | grep -v inet6 | awk '{print $$2}')
...
$(OUT_DIR)/$(PROJECT_NAME)$(EXT_NAME):
	...
	##$(info   >  CGO_ENABLED=$(CGO) GOOS=$(GOOS) GOARCH=$(GOARCH) go build $(GCFLAGS) -ldflags=$(LDFLAGS) -i -o $(OUT_DIR)/$(PROJECT_NAME)$(EXT_NAME) $(SOURCES))
